import datetime
from sklearn.metrics import roc_auc_score, mean_squared_error, precision_recall_curve, auc, r2_score, matthews_corrcoef
...
class Meter(object):
    ...
    def matthews_corrcoef(self):
        ...
        for task in range(n_tasks):
            try:
                ...
                task_y_true = y_true[:, task][task_w != 0].numpy()
                task_y_pred = y_pred[:, task][task_w != 0].numpy()
                scores.append(matthews_corrcoef(task_y_true, task_y_pred))
            except:
                print(y_pred)
                raise
        return scores
    ...
    def compute_metric(self, metric_name, reduction='mean'):
        ...
        """
        assert metric_name in ['mcc', 'roc_auc', 'l1', 'rmse', 'mae', 'roc_prc', 'r2', 'return_pred_true'], \
...
class BaseGNN(nn.Module):
    def __init__(self, gnn_out_feats, n_tasks, rgcn_drop_out=0.5, return_mol_embedding=False, return_weight=False,
        ...
        if not hasattr(classifier_hidden_feats, '__len__'):
            classifier_hidden_feats = [classifier_hidden_feats] * self.task_num
            
        if not hasattr(dropout, '__len__'):
            dropout = [dropout] * self.task_num
        
        self.fc_layers1 = nn.ModuleList([self.fc_layer(dropout[i], self.fc_in_feats, classifier_hidden_feats[i]) for i in range(len(classifier_hidden_feats))])
        self.fc_layers2 = nn.ModuleList([self.fc_layer(dropout[i], classifier_hidden_feats[i], classifier_hidden_feats[i]) for i in range(len(classifier_hidden_feats))])
        self.fc_layers3 = nn.ModuleList([self.fc_layer(dropout[i], classifier_hidden_feats[i], classifier_hidden_feats[i]) for i in range(len(classifier_hidden_feats))])
        self.output_layer1 = nn.ModuleList([self.output_layer(classifier_hidden_feats[i], 1) for i in range(len(classifier_hidden_feats))])
...
def run_an_eval_epoch_heterogeneous_with_multi_metrics(args, model, data_loader, cla_metrics, reg_metrics):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            if args['task_class'] == 'classification_regression':
                ...
                torch.cuda.empty_cache()
        if args['task_class'] == 'classification_regression':
            return [eval_meter_c.compute_metric(m) for m in cla_metrics] + \
                   [eval_meter_r.compute_metric(m) for m in reg_metrics]
        elif args['task_class'] == 'classification':
            return [eval_meter_c.compute_metric(m) for m in cla_metrics]
        else:
            return [eval_meter_r.compute_metric(m) for m in reg_metrics]
